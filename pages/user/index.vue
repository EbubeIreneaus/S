...
onMounted(() => {
  ...
  widgetScript2.type = "text/javascript";
  widgetScript2.innerHTML = JSON.stringify({
    "width": "100%",
    "height": "400",
  ...
  document.querySelector(".widget2").appendChild(widgetScript2);
});
</script>
...
<template>
  <div>
    <div class="grid md:grid-cols-2 lg:grid-cols-4 gap-5">
      <div class="bg-slate-950 shadow-md shadow-violet-300 h-36 rounded-xl">
        <div class="flex h-full items-center justify-between mx-5">
          ...
          <div class="px-3 mb-5 py-2 bg-green-600/40 rounded-md align-top">
            <i
              class="fa fa-long-arrow-right -rotate-45 text-green-400"
              aria-hidden="true"
            ></i>
          </div>
        </div>
      ...
      <div class="bg-slate-950 shadow-md shadow-violet-300 h-36 rounded-xl">
        <div class="flex h-full items-center justify-between mx-5">
          <div>
            <h2 class="text-2xl">
              {{ format_amount(account.active_investment) }}
            </h2>
      ...
      <div class="bg-slate-950 shadow-md shadow-violet-300 h-36 rounded-xl">
        <div class="flex h-full items-center justify-between mx-5">
          <div>
            <h2 class="text-2xl">
              {{ format_amount(account.pending_withdraw) }}
            </h2>
            <p>pending withdrawal</p>
          </div>
          <div class="px-3 mb-5 py-2 bg-red-600/40 rounded-md align-top">
            <i
              class="fa fa-long-arrow-right rotate-45 text-red-400"
              aria-hidden="true"
            ></i>
          </div>
        </div>
      ...
      <div class="bg-slate-950 shadow-md shadow-violet-300 h-36 rounded-xl">
        <div class="flex h-full items-center justify-between mx-5">
          <div>
            <h2 class="text-2xl">
              {{ format_amount(account.Total_earnings) }}
            </h2>
            <p>Total Earnings</p>
          </div>
          <div class="px-3 mb-5 py-2 bg-green-600/40 rounded-md align-top">
    ...
    <div class="flex gap-5 flex-wrap">
      <div
        class="shadow-md shadow-violet-300 flex flex-col gap-5 justify-center max-w-sm w-full h-80 px-5"
      >
      ...
      <div
        class="shadow-md shadow-violet-300 flex flex-col gap-5 justify-center flex-grow h-80 px-5"
      ></div>
    </div>

    <div class="tradingview-widget-container widget2 my-10">
    ...
    <div class="mt-7">
      ...
      <div class="shadow-md w-full overflow-x-auto">
        <table
          class="table-auto w-full text-left border-separate border-spacing-y-1"
          v-if="transactions"
        >
          <thead class="w-full">
            <tr class="capitalize">
          ...
          <tbody>
            <tr
              class="odd:bg-slate-800 hover:bg-slate-700"
            ...
            >
              ...
              <td
                class="px-4 bg-yellow-600/50 text-yellow-400"
                v-if="tx.status == 0"
              >
                awaiting
              </td>
              <td
                class="px-4 bg-green-600/50 text-green-400"
                v-if="tx.status == 1"
              >
                approved
              </td>
              ...
              <td
                class="px-4 bg-yellow-600/50 text-yellow-400"
                v-if="tx.progress == 'pending'"
              >
                pending
              </td>
              <td
                class="px-4 bg-green-600/50 text-green-400"
                v-if="tx.progress == 'active'"
              >
                active
              </td>
              <td
                class="px-4 bg-red-600/50 text-red-400"
                v-if="tx.progress == 'completed'"
              >
                completed
              </td>
            </tr>
